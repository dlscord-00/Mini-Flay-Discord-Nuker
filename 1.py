...
class GuildOperator:
    ...
    async def __aenter__(self) -> "GuildOperator":
...
class WorkerPool:
    def __init__(self, Operator: GuildOperator) -> None:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    return
                try:
                    await self.Operator.Request(Method, Url, Payload)
                except asyncio.CancelledError:
                    raise
                except Exception:
                    pass
...
async def DeleteChannels(Operator: GuildOperator) -> int:
    Channels = await Operator.GetChannels()
    if not Channels:
    ...
    for Channel in itertools.chain(Others, Categories):
        ...
        if not ChannelId:
            continue
        Operator.Queue.put_nowait(("DELETE", f"{ApiBase}/channels/{ChannelId}", None))
        Count += 1
    return Count


async def DeleteRoles(Operator: GuildOperator) -> int:
    Roles = await Operator.GetRoles()
    if not Roles:
    ...
    for Role in Roles:
        RoleId = SafeGet(Role, "id")
        if not RoleId or SafeGet(Role, "managed") or RoleId == Operator.GuildId:
            continue
        Operator.Queue.put_nowait(
            ("DELETE", f"{ApiBase}/guilds/{Operator.GuildId}/roles/{RoleId}", None)
        )
        Count += 1
    return Count


async def BanMembers(Operator: GuildOperator) -> int:
    MyId = SafeGet(Operator.User, "id")
    OwnerId = SafeGet(Operator.Guild, "owner_id")
    Count = 0
    async for Member in Operator.IterMembers():
        ...
        Operator.Queue.put_nowait(
            (
                "PUT",
                f"{ApiBase}/guilds/{Operator.GuildId}/bans/{UserId}",
...
async def KickMembers(Operator: GuildOperator) -> int:
    MyId = SafeGet(Operator.User, "id")
    OwnerId = SafeGet(Operator.Guild, "owner_id")
    Count = 0
    async for Member in Operator.IterMembers():
        ...
        if not UserId or UserId == MyId or UserId == OwnerId:
            continue
        Operator.Queue.put_nowait(
            ("DELETE", f"{ApiBase}/guilds/{Operator.GuildId}/members/{UserId}", None)
        )
        Count += 1
    if Count == 0:
        ErrorBox("No Members Found")
...
async def DeleteEmojis(Operator: GuildOperator) -> int:
    Response = await Operator.Request("GET", f"{ApiBase}/guilds/{Operator.GuildId}/emojis")
    ...
    for Emoji in Emojis:
        ...
        if not EmojiId:
            continue
        Operator.Queue.put_nowait(
            ("DELETE", f"{ApiBase}/guilds/{Operator.GuildId}/emojis/{EmojiId}", None)
        )
        Count += 1
    return Count


async def DeleteStickers(Operator: GuildOperator) -> int:
    Response = await Operator.Request("GET", f"{ApiBase}/guilds/{Operator.GuildId}/stickers")
    if not Response or Response.status_code != 200:
        return 0
    ...
    for Sticker in Stickers:
        ...
        if not StickerId:
            continue
        Operator.Queue.put_nowait(
            ("DELETE", f"{ApiBase}/guilds/{Operator.GuildId}/stickers/{StickerId}", None)
        )
        Count += 1
    return Count


async def DeleteInvites(Operator: GuildOperator) -> int:
    Response = await Operator.Request("GET", f"{ApiBase}/guilds/{Operator.GuildId}/invites")
    if not Response or Response.status_code != 200:
        return 0
    ...
    for Invite in Invites:
        ...
        if not Code:
            continue
        Operator.Queue.put_nowait(("DELETE", f"{ApiBase}/invites/{Code}", None))
        Count += 1
    return Count


async def DeleteWebhooks(Operator: GuildOperator) -> int:
    Response = await Operator.Request("GET", f"{ApiBase}/guilds/{Operator.GuildId}/webhooks")
    if not Response or Response.status_code != 200:
        return 0
    ...
    for Webhook in Webhooks:
        ...
        if not WebhookId:
            continue
        Operator.Queue.put_nowait(("DELETE", f"{ApiBase}/webhooks/{WebhookId}", None))
        Count += 1
    return Count
...
async def RunAction(Operator: GuildOperator, Choice: str) -> int:
    if Choice == "9":
        Total = 0
        for Key in ["1", "2", "5", "6", "7", "8", "3"]:
            _, Function = Actions[Key]
            Total += await Function(Operator)
        return Total
    _, Function = Actions[Choice]
    return await Function(Operator)


def PrintActions() -> None:
    SafePrint("")
    SafePrint(
        f"{Purple}┌─ {BrightMagenta}{Bold}Guild Operator{Reset} "
        f"{Purple}──────────────────────────────────────────┐{Reset}"
    )
...
async def Main() -> None:
    ...
    SafePrint(f"{Purple}└──────────────────────────────────────────────────────────┘{Reset}")
    Operator = GuildOperator(Token, GuildId)
    async with Operator:
        if not await Operator.ValidateAll():
            return
        PrintActions()
        ...
        Total = await RunAction(Operator, Choice)
        if Total == 0:
            return
        SafePrint("")
        SafePrint(
